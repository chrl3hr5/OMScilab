...
double* Scilab_Operation(char *objective_function, char *x0, char *A, char *b, char *Aeq, char *beq, char *lb, char *ub, char *optimization_function, double* results)
{
	/****** Starting Scilab **********/
	...
	char *input_x0 = string_append("x0 = ", x0, ";");
	printf("%s\n",input_x0);
	SendScilabJob(input_x0);
	
	/* Setting parameter values for linear constraints */
	char *input_A = string_append("A = ", A, ";");
	printf("%s\n",input_A);
	SendScilabJob(input_A);
	char *input_b = string_append("b = ", b, ";");
	printf("%s\n",input_b);
	SendScilabJob(input_b);
	
	/* Setting parameter values for linear equality constraints */
	char *input_Aeq = string_append("Aeq = ", Aeq, ";");
	printf("%s\n",input_Aeq);
	SendScilabJob(input_Aeq);
	char *input_beq = string_append("beq = ", beq, ";");
	printf("%s\n",input_beq);
	SendScilabJob(input_beq);
	
	/* Setting parameter values for upper and lower bounds of the objective function */
	char *input_lb = string_append("lb = ", lb, ";");
	printf("%s\n",input_lb);
	SendScilabJob(input_lb);
	char *input_ub = string_append("ub = ", ub, ";");
	printf("%s\n",input_ub);
	SendScilabJob(input_ub);
	
	/* Performing optimization */
	char *input_optimization_function = string_append("[xopt,fopt] = ", optimization_function, "(objective_function,x0,A,b,Aeq,beq,lb,ub);");
